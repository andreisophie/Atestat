<html>
    ...
    <?php
        ...
        $ok = $ok & validateInput("E_mail");
        $ok = $ok & validateInput("ID_specializare",1);
        $ok = $ok & validateInput("Sporuri_salariu",1);
        $ok = $ok & validateInput("Data_angajarii");
        ...
        {
            header("Location: ModificaDoctor.php?ID=".$ID);
            exit;
        }
        ...
        $ID_specializare = stripslashes(htmlspecialchars(trim($_POST["ID_specializare"])));
        $Sporuri_salariu = stripslashes(htmlspecialchars(trim($_POST["Sporuri_salariu"])));
        $Data_angajarii = stripslashes(htmlspecialchars(trim($_POST["Data_angajarii"])));
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <?php
                        ...
                        $sql = "UPDATE `doctori` SET `Nume`='".$Nume."', `Prenume`='".$Prenume."', `Sex`='".$Sex."', `Data nasterii`='".$Data_nasterii."', `CNP`='".$CNP."', 
                                `Varsta`='".$Varsta."', `E-mail`='".$E_mail."', `Telefon`='".$Telefon."', `Sporuri salariu`='".$Sporuri_salariu."', 
                                `Data angajarii`='".$Data_angajarii."', `ID_specializare`='".$ID_specializare."', `ID_judet`='".$ID_judet."', `Oras`='".$Oras."', 
                                `Strada`='".$Strada."', `Cod postal`='".$Cod_postal."', `Numar`='".$Numar."', `Bloc`='".$Bloc."', `Apartament`='".$Apartament."' WHERE ID=".$ID.";";
                        $r = mysqli_query($connection, $sql); 

                        if ($r)
                        {
                            echo "<div class='alert alert-success' style='text-align: center;'>
                                    <span class='glyphicon glyphicon-ok'></span> Doctorul a fost modificat cu succes.
                                </div>";
                            $_SESSION = array();
                        }
                        else
